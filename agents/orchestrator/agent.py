import os
import json
from typing import AsyncGenerator
from google.adk.agents import BaseAgent, LoopAgent, SequentialAgent
from google.adk.agents.remote_a2a_agent import RemoteA2aAgent
from google.adk.events import Event, EventActions
from google.adk.agents.invocation_context import InvocationContext
from google.adk.agents.callback_context import CallbackContext

from authenticated_httpx import create_authenticated_client

##Feedback Refiner
class FeedbackRefiner(BaseAgent):
    """Refines the research query using judge feedback."""

    async def _run_async_impl(self, ctx: InvocationContext):
        goal = ctx.session.state.get("user_input", "")
        feedback = ctx.session.state.get("judge_feedback", "")

        if feedback:
            refined_query = f"""
            Original Goal:
            {goal}

            Improve research based on feedback:
            {feedback}

            Focus on missing areas and risks.
            """

            ctx.session.state["refined_query"] = refined_query
            print("[FeedbackRefiner] Updated refined_query")

        yield Event(author=self.name)

# class GoalInitializer(BaseAgent):
#     async def _run_async_impl(self, ctx: InvocationContext):
#         if "user_input" not in ctx.session.state:
#             ctx.session.state["user_input"] = ctx.session.input.text
#         yield Event(author=self.name)

# --- Callbacks ---
def create_save_output_callback(key: str):
    """Creates a callback to save the agent's final response to session state."""
    def callback(callback_context: CallbackContext, **kwargs) -> None:
        ctx = callback_context
        # Find the last event from this agent that has content
        for event in reversed(ctx.session.events):
            if event.author == ctx.agent_name and event.content and event.content.parts:
                text = event.content.parts[0].text
                if text:
                    # Try to parse as JSON if it looks like it, for judge_feedback
                    if key == "judge_feedback" and text.strip().startswith("{"):
                        try:
                            ctx.state[key] = json.loads(text)
                        except json.JSONDecodeError:
                            ctx.state[key] = text
                    else:
                        ctx.state[key] = text
                    print(f"[{ctx.agent_name}] Saved output to state['{key}']")
                    return
    return callback

# --- Remote Agents ---

# Connect to the Researcher (Localhost port 8001)
researcher_url = os.environ.get("RESEARCHER_AGENT_CARD_URL", "http://localhost:8001/a2a/agent/.well-known/agent-card.json")
researcher = RemoteA2aAgent(
    name="researcher",
    agent_card=researcher_url,
    description="Performs real-time market intelligence by researching competitors, trends, and external data to generate actionable insights.",
    # IMPORTANT: Save the output to state for the Judge to see
    after_agent_callback=create_save_output_callback("research_findings"),
    # IMPORTANT: Use authenticated client for communication
    httpx_client=create_authenticated_client(researcher_url)
)

# Connect to the Judge (Localhost port 8002)
judge_url = os.environ.get("JUDGE_AGENT_CARD_URL", "http://localhost:8002/a2a/agent/.well-known/agent-card.json")
judge = RemoteA2aAgent(
    name="judge",
    agent_card=judge_url,
    description="Critically evaluates research for completeness, identifies risks, gaps, and assumptions, and ensures readiness for real-world execution.",
    after_agent_callback=create_save_output_callback("judge_feedback"),
    httpx_client=create_authenticated_client(judge_url)
)

# Content Builder (Localhost port 8003)
content_builder_url = os.environ.get("CONTENT_BUILDER_AGENT_CARD_URL", "http://localhost:8003/a2a/agent/.well-known/agent-card.json")
content_builder = RemoteA2aAgent(
    name="content_builder",
    agent_card=content_builder_url,
    description="Transforms validated insights into structured, actionable execution plans with clear steps, priorities, and outcomes.",
    httpx_client=create_authenticated_client(content_builder_url)
)

# Operator MCP Agent (Localhost port 8005)
operator_url = os.environ.get("OPERATOR_AGENT_CARD_URL", "http://localhost:8005/a2a/operator/.well-known/agent-card.json")
operator = RemoteA2aAgent(
    name="operator",
    agent_card=operator_url,
    description="Executes real-world actions like scheduling, email handling, and expense tracking.",
    httpx_client=create_authenticated_client(operator_url)
)

# --- Escalation Checker ---

class EscalationChecker(BaseAgent):
    """Checks the judge's feedback and escalates (breaks the loop) if it passed."""

    async def _run_async_impl(
        self, ctx: InvocationContext
    ) -> AsyncGenerator[Event, None]:
        # Retrieve the feedback saved by the Judge
        feedback = ctx.session.state.get("judge_feedback")
        print(f"[EscalationChecker] Feedback: {feedback}")

        # Check for 'pass' status
        is_pass = False
        if isinstance(feedback, dict) and feedback.get("status") == "pass":
            is_pass = True
        # Handle string fallback if JSON parsing failed
        elif isinstance(feedback, str) and '"status": "pass"' in feedback:
            is_pass = True

        if is_pass:
            # 'escalate=True' tells the parent LoopAgent to stop looping
            yield Event(author=self.name, actions=EventActions(escalate=True))
        else:
            # Continue the loop
            yield Event(author=self.name)

escalation_checker = EscalationChecker(name="escalation_checker")

# --- Orchestration ---

research_loop = LoopAgent(
    name="intelligence_loop",
    description="Continuously improves research until it is actionable and validated.",
    sub_agents=[
        researcher,
        judge,
        FeedbackRefiner(name="feedback_refiner"),
        escalation_checker
    ],
    max_iterations=3,
)

root_agent = SequentialAgent(
    name="venture_os_orchestrator",
    description="An autonomous CEO agent that researches, validates, and creates execution plans.",
    sub_agents=[
        # GoalInitializer(name="goal_initializer"),
        research_loop,
        content_builder
    ],
)

