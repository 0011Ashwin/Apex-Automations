...
echo "operator Agent on port 8005..."
pushd agents/operator
...
export OPERATOR_AGENT_CARD_URL=http://localhost:8005/a2a/agent/.well-known/agent-card.json
...
echo "Operator MCP: http://localhost:8005"
...
trap "kill $RESEARCHER_PID $JUDGE_PID $CONTENT_BUILDER_PID $OPERATOR_PID $ORCHESTRATOR_PID $BACKEND_PID; exit" INT
wait
